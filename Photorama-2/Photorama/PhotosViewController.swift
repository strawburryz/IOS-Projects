...
class PhotosViewController: UIViewController, UICollectionViewDelegate {
    ...
    private func updateDataSource(with predicate: NSPredicate? = nil) {
        let fetchPredicate: NSPredicate?
           ...
           if photosSegmentedControl.selectedSegmentIndex == 1 {
               // If the selected segment is 'Favorites', fetch only the favorite photos
               fetchPredicate = NSPredicate(format: "favorite == true")
           } else {
               // Otherwise, fetch all photos
               fetchPredicate = nil
           }
           
           store.fetchAllPhotos(with: fetchPredicate) { (photosResult) in
               switch photosResult {
